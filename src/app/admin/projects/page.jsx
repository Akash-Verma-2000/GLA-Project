        ...
        if (projectToDeleteId) {
            ...
            try {
                ...
                const res = await fetch(`/api/project`, {
                    ...
                    body: JSON.stringify({ _id: projectToDeleteId })
                });
            ...
            <section>
                <div className="bg-primary hidden lg:block text-white text-lg py-2 px-5 rounded-lg font-semibold  mb-5">
                    <div className="flex flex-row items-center justify-center gap-1">
                        <MdOutlineGroups /><h1>Project Management</h1>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-10 items-center justify-start gap-4 mb-10">
                    ...
                    <button className="bg-primary cursor-pointer text-white px-4 py-2 rounded-md hover:bg-blue-600 transition-colors duration-200 text-md col-span-10 md:col-span-3 xl:col-span-2" onClick={() => { router.push("/admin/projects/create") }} >
                        Add Project
                    </button>
                </div>
                ...
                <div className="hidden lg:block">
                    ...
                    {projects.length === 0 && !loading ?
                        < div className="flex items-center justify-center min-h-screen pb-[250px]">
                            <div className="text-center py-4 text-lg font-semibold text-red-500">No projects found.</div>
                        </div> : null}
                        ...
                        <table className="w-full text-left border-collapse mb-5">
                            ...
                            <tbody className="text-sm">
                                {projects.map((project, index) => (
                                    <tr key={project._id} className="hover:bg-gray-100 transition-colors duration-200">
                                        <td className="px-4 py-3 border-b border-gray-200">{project?.sno<10?"0"+project?.sno:project?.sno}.</td>
                                        <td className="px-4 py-3 border-b border-gray-200">
                                            {project.image && <img src={project.image} alt={project.title} className="w-12 h-12 object-cover rounded-md" />}
                                        </td>
                                        <td className="px-4 py-3 border-b border-gray-200">{project.title}</td>
                                        <td className="px-4 py-3 border-b border-gray-200">{project.phone}</td>
                                        <td className="px-4 py-3 border-b border-gray-200">{project.email}</td>
                                        <td className="px-4 py-3 border-b border-gray-200">
                            ...
                            {projects.map((project, index) => {
                                return <>
                                    <div key={index} className="border rounded-md border-gray-300 p-5">
                                        {project.image && <img src={project.image} alt={project.title} className="w-full h-auto mb-4 rounded-md" />}
                                        <h2 className="font-semibold text-xl">{project?.title}</h2>
                                        <p className="text-gray-600"><span className="font-semibold text-black">Phone:</span> {project?.phone}</p>
                                        <p className="text-gray-600"><span className="font-semibold text-black">Email:</span> {project?.email}</p>

                                        <div className="flex justify-between items-center gap-2 mt-5">

                                            <p className="text-gray-600"><span className="font-semibold text-black">S No:</span> {project?.sno<10?"0"+project?.sno:project?.sno}</p>
